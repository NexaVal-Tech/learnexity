...
use App\Mail\KidsEnrollmentRegistration;   // ← NEW
...
class KidsController extends Controller
{
    ...
    // Creates enrollment and sends a registration confirmation email.
    // ──────────────────────────────────────────────────────────────────────────
    public function enroll(Request $request): JsonResponse
    {
        ...
        try {
            ...
            if ($enrollmentType === 'bundle' && $dfCourse) {
                ...
                $finalPrice     = $trackCourse->getFinalPrice($sessionType, $currency, $paymentType, true);
            ...
            try {
                Mail::to($primaryEnrollment->parent_email)
                    ->send(new KidsEnrollmentRegistration($primaryEnrollment->load('course')));
            } catch (\Exception $e) {
                // Non-fatal — log but do not fail the enrollment
                Log::error('Kids registration email failed', [
                    'enrollment_id' => $primaryEnrollment->id,
                    'error'         => $e->getMessage(),
                ]);
            }
