...
class CourseResourcesController extends Controller
{
    ...
    public function getCourseResources(Request $request, string $courseId): JsonResponse
    {
        ...
        if (!$this->userCanAccessCourse($userId, $courseId)) {
            return response()->json(['error' => 'Course access required'], 403);
        }
    ...
    public function markItemCompleted(Request $request, int $itemId): JsonResponse
    {
        ...
        $materialItem = MaterialItem::with('courseMaterial')->findOrFail($itemId);
        $courseId = $materialItem->courseMaterial->course_id;
    ...
    public function downloadMaterial(Request $request, int $itemId): mixed
    {
        ...
        if (!$materialItem->file_path || !$this->storedMaterialExists($materialItem->file_path)) {
            return response()->json(['error' => 'File not found'], 404);
        }

        return $this->downloadStoredMaterial($materialItem->file_path, $materialItem->title);
    }
    ...
    private function userCanAccessCourse(int $userId, string $courseId): bool
    {
    ...
    private function storedMaterialExists(string $path): bool
    {
        return Storage::exists($path) || Storage::disk('public')->exists($path);
    }

    private function downloadStoredMaterial(string $path, string $title): mixed
    {
        if (Storage::exists($path)) {
            return Storage::download($path, $title);
        }

        return Storage::disk('public')->download($path, $title);
    }
}
