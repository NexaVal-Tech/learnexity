...
class AdminCourseResourcesController extends Controller
{
    ...
    public function deleteMaterial(string $courseId, int $materialId): JsonResponse
    {
        ...
        foreach ($material->items as $item) {
            if ($item->file_path) {
                $this->deleteStoredMaterial($item->file_path);
            }
        }
    ...
    public function addMaterialItem(Request $request, string $courseId, int $materialId): JsonResponse
    {
        $validated = $request->validate([
            ...
            'file_url' => ['nullable', 'url', 'regex:/^https?:\/\//i'],
    ...
    {
        $request->validate([
            'file' => 'required|file|mimes:pdf,doc,docx,ppt,pptx,xls,xlsx,txt,mp4,webm,mov|max:51200', // Max 50MB
        ]);
        ...
        $path = $file->store('courses/materials');
    ...
    private function deleteStoredMaterial(string $path): void
    {
