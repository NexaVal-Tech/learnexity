...
export const SignUpButton = ({ label = "Sign Up", href = "/user/auth/register" }) => {
  return (
    <Link href={href}>
      <button className="bg-[#4A3AFF] text-white px-2 py-1 rounded-full border-2 border-white font-semibold text-base md:text-lg hover:bg-gray-50 hover:text-[#4A3AFF] transition-colors flex items-center justify-center space-x-2">
        <span>{label}</span>
      </button>
    </Link>
  );
