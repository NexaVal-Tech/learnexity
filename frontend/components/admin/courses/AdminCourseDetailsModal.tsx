...
import { X, Plus, Trash2, Wrench, BookOpen, Star, Briefcase, Building2, DollarSign } from 'lucide-react';
...
const AddCourseDetailsModal: React.FC<AddCourseDetailsModalProps> = ({
  ...
  // Tools state
  const [tools, setTools] = useState<Tool[]>([{ name: '', icon: '', order: 0 }]);
  ...
  // Tools handlers
  const addTool = () => {
    setTools([...tools, { name: '', icon: '', order: tools.length }]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (validTools.length > 0) {
        promises.push(
          ...validTools.map((tool, index) =>
            api.post(`/api/admin/courses/${courseId}/details/tools`, {
              ...tool,
              order: index,
            })
          )
      ...
      if (validLearnings.length > 0) {
        promises.push(
          ...validLearnings.map((learning, index) =>
            api.post(`/api/admin/courses/${courseId}/details/learnings`, {
              ...learning,
              order: index,
            })
          )
      ...
      if (validBenefits.length > 0) {
        promises.push(
          ...validBenefits.map((benefit, index) =>
            api.post(`/api/admin/courses/${courseId}/details/benefits`, {
              ...benefit,
              order: index,
            })
          )
      ...
      if (validCareerPaths.length > 0) {
        promises.push(
          ...validCareerPaths.map((path, index) =>
            api.post(`/api/admin/courses/${courseId}/details/career-paths`, {
              ...path,
              order: index,
            })
          )
      ...
      if (validIndustries.length > 0) {
        promises.push(
          ...validIndustries.map((industry, index) =>
            api.post(`/api/admin/courses/${courseId}/details/industries`, {
              ...industry,
              order: index,
            })
          )
      ...
      if (salary.entry_level || salary.mid_level || salary.senior_level) {
        promises.push(
          api.post(`/api/admin/courses/${courseId}/details/salary`, salary)
        );
      }
                  ...
                  <div key={index} className="flex gap-3 items-start">
                    <div className="flex-1 grid grid-cols-2 gap-3">
                      <input
                        ...
                        className="px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#0F172A] focus:border-transparent"
                      />
                      <input
                        type="text"
                ...
                {learnings.map((learning, index) => (
                  <div key={index} className="flex gap-3 items-start">
                    <input
                      ...
                      className="flex-1 px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#0F172A] focus:border-transparent"
                    />
                    {learnings.length > 1 && (
                      <button
                ...
                {benefits.map((benefit, index) => (
                  <div key={index} className="border border-gray-200 rounded-xl p-4">
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#0F172A] focus:border-transparent resize-none"
                    />
                  </div>
                ))}
                <button
