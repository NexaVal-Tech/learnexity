...
export default function AdminLogin() {
  ...
  const { admin, login, error: authError, clearError, loading: authLoading } = useAdminAuth();
  ...
  // Only redirect once auth check is complete and admin is confirmed logged in
  useEffect(() => {
    if (!authLoading && admin) {
      const intendedRoute = sessionStorage.getItem('admin_intended_route');
      if (intendedRoute) {
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-[#08080c] dark:to-[#0f0f1a]">
        <svg
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-[#08080c] dark:to-[#0f0f1a] px-4">
      <div className="max-w-md w-full space-y-8 bg-white dark:bg-[#0f0f14] p-8 rounded-2xl shadow-2xl mt-20">
        {/* Logo/Header */}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <input
                ...
                name="remember-me"
                type="checkbox"
                className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 dark:border-white/20 rounded"
              />
