...
export default function LearnexityApp(): JSX.Element {
  /* ---------- STATES ---------- */
  ...
  const features: Feature[] = [
    {
      ...
      description: "We do not just teach tech skills; we help you start earning with them.",
    },
    {
      title: "Get Job Placement",
      description: "Get matched with our local and international partners for your dream job.",
    },
    {
      title: "Gain Real-World Internship Experience",
      description: "We provide hands-on experience employers are looking for.",
    },
    {
      title: "Job Support",
      description: "Resume reviews, LinkedIn optimization, interview prep, and global job access.",
    },
  ];
  ...
  const communities: Community[] = [
    { name: "AI Content Automation", whatsappLink: "https://chat.whatsapp.com/YOUR_LINK_1", images: ["/images/AI.jpeg"] },
    { name: "Video Editing & Motion Graphics", whatsappLink: "https://chat.whatsapp.com/YOUR_LINK_2", images: ["/images/VE.jpeg"] },
    { name: "AI Web Development (Vibe Coding)", whatsappLink: "https://chat.whatsapp.com/YOUR_LINK_3", images: ["/images/WD.jpeg"] },
    { name: "Social Media Management", whatsappLink: "https://chat.whatsapp.com/YOUR_LINK_4", images: ["/images/SM.jpeg"] },
    { name: "Product Management", whatsappLink: "https://chat.whatsapp.com/YOUR_LINK_5", images: ["/images/PM.jpeg"] },
    { name: "UI/UX Design", whatsappLink: "https://chat.whatsapp.com/YOUR_LINK_6", images: ["/images/UI.jpeg"] },
    { name: "Digital Marketing", whatsappLink: "https://chat.whatsapp.com/YOUR_LINK_7", images: ["/images/DM.jpeg"] },
    { name: "Frontend Development", whatsappLink: "https://chat.whatsapp.com/YOUR_LINK_8", images: ["/images/FD.jpeg"] },
    { name: "Backend Development", whatsappLink: "https://chat.whatsapp.com/YOUR_LINK_9", images: ["/images/BD.jpeg"] },
  ];
  ...
  return (
    <main className="min-h-screen bg-[#ececef] pb-20">
      ...
      <header className="fixed top-4 left-0 w-full z-50">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-3 bg-white/90 backdrop-blur-md shadow-lg rounded-2xl">
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => { setShareOpen(!shareOpen); setMenuOpen(false); }}>
              <Share2 className="text-purple-600" />
            </button>

            <button onClick={() => { setMenuOpen(!menuOpen); setShareOpen(false); }}>
      ...
      {menuOpen && (
        <div className="fixed right-6 top-24 w-80 z-50 bg-white rounded-3xl p-5 shadow-2xl">
          <button onClick={() => setMenuOpen(false)} className="absolute top-3 right-3">
            <X size={16} />
          </button>

          <h3 className="font-bold">Quick Menu</h3>

          <Link href="/LearnexityRegistration" className="block mt-4 text-purple-700">
            Register
          </Link>

          <a href={links[1].link} target="_blank" className="block mt-3 text-green-600">
            WhatsApp Community
          </a>
        </div>
      )}
      ...
      {shareOpen && (
        <div className="fixed right-6 top-24 w-80 z-50 bg-white rounded-3xl p-5 shadow-2xl">
          <button onClick={() => setShareOpen(false)} className="absolute top-3 right-3">
            <X size={16} />
          </button>

          <h3 className="font-bold">Share Learnexity</h3>

          <button onClick={copyLink} className="mt-4 text-purple-700">
      ...
      <div className="relative h-[300px] bg-cover bg-center"
        style={{ backgroundImage: "url('https://images.unsplash.com/photo-1551288049-bebda4e38f71?q=80&w=1600&auto=format&fit=crop')" }}
      />

      <section className="relative -mt-40 flex justify-center px-4">
        <div className="w-full max-w-4xl bg-white rounded-3xl p-8 shadow-lg">
          ...
          <p className="text-gray-700">
            Learn high-income digital skills and get global opportunities.
          </p>
          ...
          <div className="flex gap-3 mt-6">
            {socialLinks.map((s, i) => {
              const Icon = s.icon;
              return (
                <a key={i} href={s.link} className="bg-purple-600 text-white p-3 rounded-full">
                  <Icon size={18} />
                </a>
              );
            })}
          ...
          <div className="grid md:grid-cols-2 gap-4 mt-8">
            {features.map((f, i) => (
              <div key={i} className="bg-purple-600 text-white p-4 rounded-xl">
                <button onClick={() => setActiveCard(activeCard === i ? null : i)}>
                  {f.title}
                </button>

                {activeCard === i && (
                  <p className="mt-2 bg-white text-black p-3 rounded-xl">
                    {f.description}
                  </p>
                )}
              </div>
      ...
      <section className="bg-gray-100 py-12 px-4 mt-10">
        <div className="max-w-3xl mx-auto space-y-4">
          {communities.map((c, i) => (
            <button
              key={i}
              onClick={() => setSelectedCommunity(c)}
              className="w-full bg-purple-600 text-white py-5 rounded-full text-xl"
            >
              {c.name}
            </button>
          ))}
      ...
      {selectedCommunity && (
        ...
        >
          <div onClick={(e) => e.stopPropagation()} className="relative max-w-md w-full">
            <img src={selectedCommunity.images[0]} className="w-full h-[450px] object-cover" />

            <a
              href={selectedCommunity.whatsappLink}
              target="_blank"
              className="absolute bottom-6 right-4 bg-purple-600 text-white px-4 py-2 rounded-full"
            >
              Join WhatsApp
            </a>
          </div>
