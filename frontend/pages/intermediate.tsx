...
export default function IntermediatePage() {
  ...
  const fetchIntermediateCourses = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`${API_URL}/api/courses/by-track?track[]=intermediate`, {
  ...
  const handlePurchase = useCallback(
    async (course: Course, e: React.MouseEvent) => {
      ...
      if (!user) {
        router.push(`/user/auth/login?redirect=/intermediate`);
        return;
      }
              ...
              <div className="max-w-screen-xl mx-auto">
                ...
                {error && !loading && (
                  <div className="text-center py-20">
                    ...
                    <button
                      onClick={fetchIntermediateCourses}
